...
package body Implementation_Dependent_Routines is
   ...
   begin
      --  if
      --    file_names.exists (a_strings.to_a (in_directory & "/" &
      --    xform_files_subdirectory))
      --  then
      raise Program_Error;
      return False;
   end Anna_Mklib_Done;
   ...
   function Make_Xform_File_Name (Directory : String;
                                  File_Name : String) return String
   is
      --  Returns the string corresponding to a file name that can be used to
      --  unambiguously refer to FILE_NAME in
      --  DIRECTORY/xform_files_subdirectory.
   begin
   ...
   end Predefined_Semantics_Directory;
