...
package body Prolog.Database is
   ...
   begin --  findclause
      if Cl = null then
         ...
         loop
            if
              (Cl.Dbase /= Dbase) or
                ((Cl.Keyval /= 0) and (K /= 0) and (Cl.Keyval /= K))
            then
               Cl := Cl.Next;
            else
               Ok := True;
      ...
      begin
         if (V1.Tag = Vart) and then (V1.Id = Anon_String) then
            ...
            if Found then
               ...
               Skel_Garb.Get (V2);
               if Varmap (N).Sourcevar.Tag = Vart then
                  V2.all := Node'(Skelt, null, Heapf, 0, null,
                                  Varmap (N).Address,
      ...
      begin --  Skeleton
         ...
         case Y.Tag is
            ...
            when Skelt =>
               if Y.Anont then
                  Skel_Garb.Get (Temp);
                  Temp.all := Node'(Skelt, null, Heapf, 0, null, 0,
                                    Anon_String, True);
                  return Temp;
               else
                  Skelvar (Y, Z);
   ...
   begin --  MAKECLAUSE
      ...
      if
        Get_Info (Newhead.Name).Sys and not
          (Mode = Sysm and
             Get_Info (Newhead.Name).Pclass = Normp)
      then
