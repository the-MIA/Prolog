...
function Call_Eval_Pred (
   ...
   Routine : Evalpred;
   Arity   : Integer;
   Dbase   : Integer_List) return Boolean is
       ...
       if Isatom (Argval(1)) then
           ...
           while Isfunc (X, Consa, 2) loop
               ...
               if  (Ch < Character'Pos(Character'First)) or
                   (Ch > Character'Pos(Character'Last)) then
...
begin --  CallEvalPred
    ...
    case Routine is
        ...
        when Isr => Result := Intresult (Argval(1), E, Evaluate(Argval(2),E,0));
                              --  Right here
        ...
        when Seer => Result := Seefile (Writeatom(Argval(1).Name)); --  In IO

        when Seenr => Result := Seenfile; --  In IO

        when Tellr => Result := Tellfile (Writeatom(Argval(1).Name)); --  In IO
