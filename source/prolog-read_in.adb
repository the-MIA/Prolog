...
package body Prolog.Read_In is
   ...
   begin
      Tf :=
        Get_Info (A).all.Oclass = Fxo  or else
          Get_Info (A).Oclass = Xfxo   or else
   ...
   function Read_In return Term is
      ...
      Hiprec  : Prec;
      Loprec  : Prec; --  Current High and Low Precedences.
      ...
      procedure Stow_Atom (A : Atom) is
         ...
         begin
            ...
            if
              ((Lp < Loprec) or (P > Subprec)) and
                (not ((Context = Outer_K) or
                        (Context = Inner_K) or (Context = Curlyk)))
            then
